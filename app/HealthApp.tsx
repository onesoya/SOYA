...
function ProfileGoalSheet({ profile, latestBody, today, close, save }: { profile: AppState["profile"]; latestBody?: BodyRecord; today: string; close: () => void; save: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const [bodyFatChange, setBodyFatChange] = useState("0");
  const [muscleChange, setMuscleChange] = useState("0");
  const bodyFatDelta = Number(bodyFatChange) || 0;
  const muscleDelta = Number(muscleChange) || 0;
  const bodyFatTarget = latestBody ? roundNutrient(latestBody.bodyFatMass + bodyFatDelta) : undefined;
  const muscleTarget = latestBody ? roundNutrient(latestBody.skeletalMuscle + muscleDelta) : undefined;
  ...
  return <Sheet title="목표와 모드" close={close}><form className="form-stack profile-goal-form" onSubmit={save}>
    ...
    <section className="body-goal-fields"><strong>기간 동안의 변화 목표</strong><div className="two-fields goal-change-fields"><Field label="체지방량 변화 (kg)"><input type="number" inputMode="decimal" name="targetBodyFatChange" step="0.1" value={bodyFatChange} onChange={(event) => setBodyFatChange(event.target.value)} required /></Field><Field label="골격근량 변화 (kg)"><input type="number" inputMode="decimal" name="targetMuscleChange" step="0.1" value={muscleChange} onChange={(event) => setMuscleChange(event.target.value)} required /></Field></div>{latestBody ? <div className="goal-target-preview"><article><span>체지방량 예상 목표</span><strong>{bodyFatTarget}kg</strong><small>{latestBody.bodyFatMass}kg {signed(bodyFatDelta)}kg</small></article><article><span>골격근량 예상 목표</span><strong>{muscleTarget}kg</strong><small>{latestBody.skeletalMuscle}kg {signed(muscleDelta)}kg</small></article><em>최근 측정 {latestBody.date.replaceAll("-", ".")}</em></div> : <div className="goal-current-body empty"><span>최근 인바디를 기록하면 예상 목표값도 함께 보여드려요.</span></div>}</section>
