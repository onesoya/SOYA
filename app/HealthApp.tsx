...
function menstrualPrediction(entries: CycleEntry[], today: string) {
  ...
  if (!lastStart) return { cycleLength, lastStart: undefined, nextPeriod: undefined, nextOvulation: undefined, fertileDates, ovulationDates, basedOnCycles: 0 };
  ...
  for (let index = 0; index < 18; index += 1) {
    const ovulation = addDays(projectedStart, cycleLength - 14);
...
function assessNutrition(total: NutritionTotal, mealCount: number, complete: boolean, profile: AppState["profile"], goal: AppState["nutritionGoal"], date: string): "none" | "balanced" | "partial" | "attention" {
  ...
  if (total.sugar > goal.sugarMax * 1.25 || total.calories > goal.caloriesMax * 1.15 || total.protein < goal.proteinMin * 0.65) return "attention";
  if (total.calories >= goal.caloriesMin && total.calories <= goal.caloriesMax && total.protein >= goal.proteinMin && total.sugar <= goal.sugarMax && total.fiber >= goal.fiberMin) return "balanced";
  return "partial";
}
...
export function HealthApp() {
  ...
  return (
    <div className="app-shell">
      ...
      {modal === "profile-goal" && <ProfileGoalSheet profile={state.profile} today={today} close={() => setModal(null)} save={saveProfileGoal} />}
...
function TodayView(props: TodayViewProps) {
  const { state, today, todayBody, nutrition, completedCount, totalCount, nextAction, mealActual, mealPlan, actualWorkouts, plannedWorkout, openNextAction, skipNextAction, setModal, setTab, openMeal, openWorkout, updateTravelDayLevel } = props;
...
function FoodView({ state, today, openMeal, deleteMeal, openGoal, openLibrary, updateTravelDayLevel }: { state: AppState; today: string; openMeal: (kind: EntryKind, presetType?: MealType, draft?: MealEntry, date?: string) => void; deleteMeal: (entry: MealEntry) => void; openGoal: () => void; openLibrary: () => void; updateTravelDayLevel: (date: string, level: TravelLevel) => void }) {
  ...
  const mealStatus = (date: string) => {
    ...
    return assessNutrition(nutritionTotal(meals), meals.length, new Set(meals.map((item) => item.mealType)).size >= 3, state.profile, state.nutritionGoal, date);
  };
...
function WorkoutView({ state, today, openWorkout, deleteWorkout, openGoal, updateTravelDayLevel }: { state: AppState; today: string; openWorkout: (kind: EntryKind, draft?: WorkoutEntry, presetType?: WorkoutEntry["type"]) => void; deleteWorkout: (entry: WorkoutEntry) => void; openGoal: () => void; updateTravelDayLevel: (date: string, level: TravelLevel) => void }) {
  const [selectedMonth, setSelectedMonth] = useState(today.slice(0, 7));
  const [selectedDate, setSelectedDate] = useState(today);
...
function MenstrualView({ state, today, openRecord, editRange, deleteRange }: { state: AppState; today: string; openRecord: (date: string) => void; editRange: (date: string) => void; deleteRange: (date: string) => void }) {
  const [selectedMonth, setSelectedMonth] = useState(today.slice(0, 7));
  const [selectedDate, setSelectedDate] = useState(today);
  const cells = monthCells(`${selectedMonth}-01`);
  ...
  return <div className="section-stack menstrual-view">
    <section className="card pixel-calendar-card menstrual-calendar-card">
      ...
      <div className="month-grid">{cells.map((date, index) => {
        ...
        const love = (entry?.sexCount ?? 0) > 0 ? entry?.contraception === "피임하지 않음" ? "filled" : entry?.contraception === "피임함" ? "outline" : "" : "";
    ...
    <div className="metric-grid menstrual-metrics"><MetricCard label="평균 주기" value={prediction.lastStart ? String(prediction.cycleLength) : "-"} unit="일" hint={prediction.basedOnCycles ? `최근 ${prediction.basedOnCycles + 1}주기 기준` : prediction.lastStart ? "기록 1회 · 28일 기준" : "본 출혈 기록 필요"} /><MetricCard label="다음 예상 배란일" value={prediction.nextOvulation ? prediction.nextOvulation.slice(5).replace("-", ".") : "-"} unit="" hint={prediction.nextPeriod ? `다음 월경 예상 ${prediction.nextPeriod.slice(5).replace("-", ".")}` : "기록이 쌓이면 계산"} /><MetricCard label="기록된 주기" value={histories.length ? String(histories.length) : "-"} unit="회" hint={histories.length ? `${histories.at(-1)?.start.slice(0, 4)}년부터` : "본 출혈 기록 필요"} /><MetricCard label="평균 본 출혈" value={averageMainBleeding ? String(averageMainBleeding) : "-"} unit="일" hint="전체 주기 기준" /></div>
    <section className="card cycle-history-card"><CardTitle title="주기별 기록" aside={cycleYears.length ? <select className="cycle-year-select" value={cycleYear} onChange={(event) => setCycleYear(event.target.value)} aria-label="주기 기록 연도"><option value="전체">전체</option>{cycleYears.map((year) => <option key={year} value={year}>{year}년</option>)}</select> : undefined} />
